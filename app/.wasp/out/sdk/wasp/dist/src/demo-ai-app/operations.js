...
export const generateGptResponse = async (rawArgs, context) => {
    ...
    const { projectGoal } = ensureArgsSchemaOrThrowHttpError(generateGptResponseInputSchema, rawArgs);
    console.log("Calling NVIDIA NIM API");
    const breakdown = await generateProjectBreakdown(projectGoal);
    if (breakdown === null) {
        throw new HttpError(500, "Encountered a problem in communication with AI service");
    }
...
async function generateProjectBreakdown(projectGoal) {
