...
export default function ZeroclawPage() {
    ...
    if (!instance) {
        return (<div className="mx-auto max-w-2xl px-6 py-16 text-center">
        <Card>
    ...
    return (<div className="mx-auto max-w-2xl px-6 py-10">
      <Card className="mb-6">
        <CardHeader>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          {instance.dashboardUrl && (<div className="grid grid-cols-2 gap-2">
          ...
          {instance.provisioningError && (<div className="grid grid-cols-2 gap-2">
              <span className="text-muted-foreground">错误信息</span>
              <span className="text-red-600 text-xs">{instance.provisioningError}</span>
            </div>)}
          {instance.status === "active" && instance.dashboardUrl && (<div className="pt-2">
          ...
          {instance.status === "failed" && (<div className="pt-2">
              <Button className="w-full" variant="outline" disabled={isRetrying} onClick={async () => {
                setIsRetrying(true);
                try {
                    await retryNow(undefined);
                    refetch();
                }
                finally {
                    setIsRetrying(false);
      ...
      {instance.status === "active" && (<Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="apiKey" type="password" placeholder="输入您的 API Key" value={apiKey} onChange={(e) => setApiKey(e.target.value)}/>
            </div>
            <Button className="w-full" disabled={isSavingKey || !apiKey} onClick={async () => {
                setIsSavingKey(true);
                try {
...
//# sourceMappingURL=ZeroclawPage.jsx.map