...
import {
  type GetZeroclawInstance,
  type SetZeroclawApiKey,
  type RetryProvision,
...
export const getZeroclawInstance: GetZeroclawInstance<
  void,
  ZeroclawInstance | null
> = async (_args, context) => {
  if (!context.user) {
...
export const setZeroclawApiKey: SetZeroclawApiKey<
  z.infer<typeof setApiKeySchema>,
  { success: boolean }
...
export const retryProvision: RetryProvision<
  void,
  { success: boolean }
> = async (_args, context) => {
  if (!context.user) {
