...
async function handleInvoicePaid(
  ...
  switch (paymentPlanId) {
    ...
    case PaymentPlanId.Hobby: {
      ...
      try {
        await provisionZeroclawJob.submit({ userId: user.id });
      } catch (jobError) {
        console.error("Failed to submit provisionZeroclawJob:", jobError);
      }
      break;
    }
    default:
...
async function handleCustomerSubscriptionDeleted(
  ...
  try {
    const instance = await prisma.zeroclawInstance.findUnique({
      where: { userId: user.id },
    });
    if (instance) {
      await prisma.zeroclawInstance.update({
        where: { id: instance.id },
        data: { status: "suspended" },
      });
      try {
        await deprovisionInstance(instance.instanceId);
      } catch (deprovisionError) {
        console.error("Failed to deprovision ZeroClaw instance:", deprovisionError);
      }
    }
  } catch (error) {
    console.error("Error handling ZeroClaw deprovision:", error);
  }
}
